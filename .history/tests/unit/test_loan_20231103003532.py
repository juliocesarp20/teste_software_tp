...
def test_make_payment(user_fixture):
    ...
    assert  pytest.approx(payment_amount, abs=1e-2) ==  pytest.approx(loan.paid_amount, abs=1e-2)
    assert  pytest.approx(amount - payment_amount, abs=1e-2) == pytest.approx(loan.remaining_balance , abs=1e-2)
