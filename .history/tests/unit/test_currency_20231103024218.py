...
def test_convert_to_invalid_target():
    brl = MockCurrency(0.2, "BRL")
    amount = 100
    with pytest.raises(ValueError, match="Target currency must be an instance of Currency."):
        brl.convert_to("EUR", amount)

def test_convert_to_negative_amount():
    eur = MockCurrency(1.1, "EUR")
    negative = MockCurrency(50,"ARG")
    with pytest.raises(ValueError, match="Amount cannot be negative or zero when converting currencies."):
